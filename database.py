...
def init_db():
    ...
    if faq_cnt == 0:
        initial_faqs = [
            ...
            ("Alloma AI yordamchisi qanday ishlaydi va uning ovozlari qanday farqlanadi?", "Alloma AI — bolalar bilan sof o'zbek tilida ovozli muloqot qiluvchi sun'iy intellekt tyutori. U bolaning yoshiga moslashadi: 3-7 yoshdagi bolalar uchun quvnoq va sodda tilda ertaknamo gapirsa, 7-11 yoshdagi bolalar uchun mantiqiy, aniq va chuqurroq tushuntirishlar beradi.", "active", 2),
            ("Uran sayyorasida ingliz tilini qanday o'rganish mumkin?", "Uran (Nutq va Til) sayyorasida 10 dan ortiq qiziqarli mavzular (mevalar, hayvonlar, ranglar, maktab va h.k.) bo'yicha inglizcha so'zlar, to'g'ri talaffuz va misol gaplar o'rgatiladi. So'zlar o'rganib bo'lingach, 4 ta variantli test orqali bilimlar mustahkamlanadi va yulduzchalar beriladi.", "active", 3),
            ("Ota-onalar farzandining ta'lim jarayonini qanday nazorat qila oladi?", "Ota-onalar shaxsiy kabineti orqali farzandining qaysi sayyoralarda qancha vaqt o'tkazgani, qaysi mavzularni o'zlashtirgani, AI bilan nimalar haqida suhbatlashgani va erishgan yutuqlari (yulduzchalar, ballar) statistikasini real vaqtda kuzatib borishlari mumkin.", "active", 4),
            ("Mobil ilovadan internet bo'lmaganda ham foydalansa bo'ladimi (Offline rejim)?", "Ha, avval yuklab olingan so'zlar, ertaklar, rasmlar va ko'plab interaktiv o'yinlardan internetsiz oflayn rejimda ham foydalanish mumkin. Faqatgina real vaqtda AI bilan suhbatlashish uchun internet talab etiladi.", "active", 5),
            ("Bolalar uchun xavfsizlik qanday ta'minlangan (Safe Kids AI)?", "Platformada qat'iy xavfsizlik filtrlari o'rnatilgan. Sun'iy intellekt faqatgina bolalar yoshiga mos, tarbiyaviy va ta'limiy mavzularda suhbatlashadi. Har qanday zararli, noo'rin yoki tajovuzkor kontent to'liq bloklanadi.", "active", 6),
            ("4 xonali PIN-kod nima uchun kerak?", "4 xonali PIN-kod ota-onalar profilini bolalardan himoyalash, obuna va to'lovlarni boshqarish hamda shaxsiy ma'lumotlar xavfsizligini ta'minlash uchun xizmat qiladi.", "active", 7),
            ("Obuna bo'lish qanday amalga oshiriladi va to'lov turlari qaysilar?", "Platformaga oylik, 6 oylik yoki yillik obuna bo'lish mumkin. To'lovlar Payme, Click va Uzum Bank kabi milliy to'lov tizimlari orqali xavfsiz va bir zumda amalga oshiriladi.", "active", 8),
            ("Bir nechta farzand uchun bitta hisobdan foydalanish mumkinmi?", "Ha, ota-ona o'z hisobida har bir farzandi uchun alohida profil (ismi, yoshi va qiziqishlari) yaratishi mumkin. Har bir bolaning yutuqlari va o'rganish darajasi mustaqil saqlanadi.", "active", 9),
            ("Qanday qurilmalarda ishlaydi (Android / iOS / Web)?", "Kichik Alloma barcha zamonaviy Android va iOS (iPhone/iPad) smartfon hamda planshetlarida, shuningdek kompyuter brauzerida to'liq va qulay ishlaydi.", "active", 10),
            ("Kunlik foydalanish vaqtiga cheklov qo'yish mumkinmi (Screen Time)?", "Ha, ota-onalar kabinetida bolaning ekran oldida o'tkazadigan vaqtiga (masalan, kuniga 30 daqiqa yoki 1 soat) cheklov o'rnatish imkoniyati mavjud. Belgilangan vaqt tugagach, ilova bolani charchaganini eslatib, dam olishga undaydi.", "active", 11),
            ("Texnik yordam yoki savollar bo'yicha kimga murojaat qilish kerak?", "Ilovadagi 'Bog'lanish' bo'limi, vebsaytdagi aloqa formasi yoki rasmiy @kichikalloma_support Telegram boti orqali 24/7 bizning qo'llab-quvvatlash xizmatimizga murojaat qilishingiz mumkin.", "active", 12)
        ]
        cursor.executemany(
